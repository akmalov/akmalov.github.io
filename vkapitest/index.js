...
function vkApi(method, options) {
    (0, _assign2.default)(options, { v: '5.65' });
...
function vkInit() {
    // Инициализация приложения при первом подключении
    return new _promise2.default(function (resolve, reject) {
        VK.init({ apiId: 6059604 });
...
function vkInitGetStatus() {
    // Инициализация при последующих подключениях
    return new _promise2.default(function (resolve, reject) {
        VK.init({ apiId: 6059604 });
        VK.Auth.getLoginStatus(function (response) {
            // Проверка статуса
...
var template = function template(item) {
    return '\n    <li data-id="' + item.id + '">\n        <img class="main__photo" src="' + (item.photo_200 ? item.photo_200 : 'https://vk.com/images/deactivated_200.png') + '" alt="friend photo">\n        <p>' + item.first_name + ' ' + item.last_name + '</p>\n    </li>';
};

var render = function render() {
    // Функция рендера списка
    document.querySelector('.main__friends').innerHTML = friends.map(function (item) {
...
document.querySelector('.auth').addEventListener('click', function (e) {
    e.preventDefault();
    vkInit().then(function () {
        document.querySelector('.button-container').className = "button-container button-container_hidden";
    }).then(function () {
        return vkApi('users.get', { name_case: 'gen' });
    }).then(function (response) {
        header.textContent = '\u0414\u0440\u0443\u0437\u044C\u044F ' + response[0].first_name + ' ' + response[0].last_name;
    }).then(function () {
        return vkApi('friends.get', { fields: 'photo_200' });
    }).then(function (response) {
        return friends = response.items;
    }).then(render);
});

vkInitGetStatus().then(function () {
    document.querySelector('.button-container').className = "button-container button-container_hidden";
}).then(function () {
    return vkApi('users.get', { name_case: 'gen' });
}).then(function (response) {
    ...
    return vkApi('friends.get', { fields: 'photo_200', count: 5, order: 'random' });
}).then(function (response) {
    return friends = response.items;
}).then(render);
